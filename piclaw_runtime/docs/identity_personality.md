...
| **meta.json** | `self_summary`, `writing_style` | `self_summary`: one-line “who I am” (or set via chat: set_self_summary). `writing_style`: how to write replies (tone, formality). You can say in chat e.g. “write more formally” or “be more casual” and the agent will update it with the set_writing_style tool. |
